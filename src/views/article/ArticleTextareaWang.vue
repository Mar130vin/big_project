...
export default {
  ...
  methods: {
    seteditor () {
      ...
      editor.customConfig.uploadImgServer = 'http://106.15.186.177:8090/DrugRehabilitationSystem/api/file/uploadFile'// 配置服务器端地址
      ...
      editor.customConfig.customUploadImg = function (files, insert) {
        ...
        that.axios({ // 上传图片到后台
          method: 'post',
          url: that.common.getApi() + '/sys/api/message/uploadFile',
          data: formData
        }).then(res => {
          if (res.data.success) {
            console.log(res.data.obj)

            const imgUrl = res.data.obj.savePath // 后台返回的文件路径
